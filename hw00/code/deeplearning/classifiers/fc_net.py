...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # Forward into first layer: affine - relu
        hidden_out, cache_hidden = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        # Forward into second layer: affine
        scores, cache_scores = affine_forward(hidden_out, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        # Compute softmax loss and gradient on scores
        loss, dscores = softmax_loss(scores, y)
        # Add L2 regularization to the loss (with factor 0.5)
        loss += 0.5 * self.reg * (np.sum(self.params['W1'] * self.params['W1']) +
                                  np.sum(self.params['W2'] * self.params['W2']))

        # Backprop into second layer
        dhidden, dW2, db2 = affine_backward(dscores, cache_scores)
        # Backprop into first layer (affine-relu)
        dX, dW1, db1 = affine_relu_backward(dhidden, cache_hidden)

        # Store gradients, including regularization gradient
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dropout_caches = {}
        out = X
        # Iterate over the first (num_layers - 1) layers
        for i in range(1, self.num_layers):
            ...
            if self.use_batchnorm:
                ...
                out, cache = affine_forward(out, Wi, bi)
                out, cache_bn = batchnorm_forward(out, gammai, betai, self.bn_params[i - 1])
                out, cache_relu = relu_forward(out)
                caches[i] = (cache, cache_bn, cache_relu)
            else:
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.use_batchnorm:
                cache, cache_bn, cache_relu = caches[i]
